describe("Módulo Paper: Creación y Plugins", () => {
  ...
  it("Debe crear un servidor Paper e instalar un Plugin mediante la UI de Modrinth", () => {
    ...
    cy.get('[data-cy="wizard-server-name-input"]').type("Cypress Paper Server");
    cy.get('[data-cy="wizard-software-paper"]').click();
    ...
    cy.get('[data-cy="modlist-search-input"]').type("EssentialsX");
    cy.get('[data-cy="modlist-search-btn"]').click();
    cy.get('[data-cy="store-item-essentialsx"]', { timeout: 15000 }).should("be.visible");
    cy.get('[data-cy="store-item-essentialsx"]').parents('.bg-surface').find('[data-cy="modlist-install-btn"]').click();
    cy.get('[data-cy="store-item-essentialsx"]').parents('.bg-surface').find('[data-cy="modlist-installed-badge"]', { timeout: 30000 }).should("be.visible");
    cy.get('[data-cy="modlist-installed-tab"]').click();
    cy.get('[data-cy="modlist-item-essentialsx"]', { timeout: 5000 }).should("be.visible");
  });
});
